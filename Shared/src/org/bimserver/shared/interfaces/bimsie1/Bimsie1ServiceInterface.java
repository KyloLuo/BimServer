...
public interface Bimsie1ServiceInterface extends PublicInterface {
	 ...
	 * 
	 * @param poid
	 *            The Project's ObjectID
	 * @param comment
	 *            A comment
	 * @param deserializerOid
	 *            ObjectId of the deserializer to use, use getAllDeserializers
	 *            to get a list of available deserializers
	 * @param fileSize
	 *            The size of the file in bytes
	 * @param ifcFile
	 *            The actual file
	 * @param sync
	 *            Whether the call should return immediately (async) or wait for
	 *            completion (sync)
	 * @return A topicId, which you can use for the
	 *         Bimsie1NotificationRegistryInterface.getProgress method
	...
	Long checkin(
			...
			@WebParam(name = "sync", partName = "checkin.sync") Boolean sync)
	...
	Long checkinFromUrl(
			...
			@WebParam(name = "sync", partName = "checkinFromUrl.sync") Boolean sync)
			throws ServerException, UserException;
	 ...
	 * 
	 * @param actionId
	 *            The actionId returned by one of the download or checkout
	 *            methods
	...
	void terminateLongRunningAction(
			@WebParam(name = "actionId", partName = "getDownloadData.actionId") Long actionId)
			throws ServerException, UserException;

	/**
	 * Checkout an existing model, checkout is the same as download, except a
	 * "checkout" will tell the server and other users you are working on it
	 * 
	 * @param roid
	 *            Revision ObjectID
	 * @param serializerOid
	 *            ObjectId of the serializer to use, use getAllSerializers to
	 *            find availble serializeres
	 * @param sync
	 *            Whether to return immediately (async) or wait for completion
	 *            (sync)
	 * @return A topicId, which you can use for the
	 *         Bimsie1NotificationRegistryInterface.getProgress method
	 * @throws ServerException
	...
	Long checkout(
			...
			@WebParam(name = "sync", partName = "checkout.sync") Boolean sync)
			throws ServerException, UserException;
	 ...
	 *            find availble serializeres
	 * @param showOwn
	 *            Whether to return revisions created by the current user
	 * @param sync
	 *            Whether to return immediately (async) or wait for completion
	 *            (sync)
	 * @return A topicId, which you can use for the
	...
	Long download(
			...
			@WebParam(name = "sync", partName = "download.sync") Boolean sync)
			throws ServerException, UserException;

	/**
	 * Download a model in a serialized format by giving a set of revisions and
	 * a set of Object IDs
	 * 
	 * @param roids
	 *            A set of Revision ObjectIDs
	 * @param oids
	 *            A set of ObjectIDs
	 * @param serializerOid
	 *            ObjectId of the serializer to use, use getAllSerializers to
	...
	Long downloadByOids(
			...
			@WebParam(name = "deep", partName = "downloadCompareResults.deep") Boolean deep)
			throws ServerException, UserException;

	/**
	 * Download a model in serialized format by giving a set of revisions and a
	 * set of class names to filter on
	 * 
	 * @param roids
	 *            A set of Revision ObjectIDs
	 * @param classNames
	 *            A set of class names to filter on (e.g. "IfcWindow")
	 * @param serializerOid
	 *            ObjectId of the serializer to use, use getAllSerializers to
	 *            find availble serializeres
	 * @param includeAllSubtypes
	 *            Whether to query all (recursive) subtypes of each gives class
	 * @param sync
	 *            Whether to return immediately (async) or wait for completion
	 *            (sync)
	 * @return A topicId, which you can use for the
	...
	Long downloadByTypes(
			...
			@WebParam(name = "sync", partName = "downloadByTypes.sync") Boolean sync)
			throws ServerException, UserException;
	 ...
	 *            A set of IFC guids
	 * @param serializerOid
	 *            OID of the serializer to use
	 * @param sync
	 *            Whether to return immediately (async) or wait for completion
	 *            (sync)
	 * @return An id, which you can use for the getDownloadState and
	 *         getDownloadData methods
	 * @throws ServerException
	 *             , UserException
	 */
	...
	Long downloadByGuids(
			...
			@WebParam(name = "sync", partName = "downloadByGuids.sync") Boolean sync)
			throws ServerException, UserException;

	/**
	 * Download a model in a serialized format by giving a set of revisions and
	 * a set of names to filter on
	 * 
	 * @param roids
	 *            A set of Revision ObjectIDs
	 * @param names
	 *            A set of names, the names should be exact matches for now
	 * @param serializerOid
	 *            OID of the serializer to use
	 * @param sync
	 *            Whether to return immediately (async) or wait for completion
	...
	Long downloadByNames(
			...
			@WebParam(name = "sync", partName = "downloadByNames.sync") Boolean sync)
			throws ServerException, UserException;
	...
	Long downloadRevisions(
			...
			@WebParam(name = "sync", partName = "downloadRevisions.sync") Boolean sync)
			throws ServerException, UserException;

	/**
	 * @param roid
	 *            ObjectID of the Revision to perform this query on
	 * @param code
	 *            The Java code, should be an implementation of the
	...
	Long downloadQuery(
			...
			@WebParam(name = "serializerOid", partName = "downloadQuery.serializerOid") Long serializerOid)
			throws ServerException, UserException;
	...
	SSerializerPluginConfiguration getSerializerByName(
			@WebParam(name = "serializerName", partName = "getSerializerByName.serializerName") String serializerName)
			throws ServerException, UserException;

	/**
	 * @param oid
	 *            ObjectID of the Deserializer
	 * @return Deserializer
	 * @throws ServerException
	 *             , UserException
	 */
	...
	SDeserializerPluginConfiguration getDeserializerById(
			@WebParam(name = "oid", partName = "getDeserializerById.oid") Long oid)
			throws ServerException, UserException;

	/**
	 * @param deserializerName
	...
	SDeserializerPluginConfiguration getDeserializerByName(
			@WebParam(name = "deserializerName", partName = "getDeserializerByName.deserializerName") String deserializerName)
			throws ServerException, UserException;

	/**
	 * @param oid
	 *            ObjectID of the Serializer
	 * @return Serializer
	 * @throws ServerException
	 *             , UserException
	 */
	...
	SSerializerPluginConfiguration getSerializerById(
			@WebParam(name = "oid", partName = "getSerializerById.oid") Long oid)
			throws ServerException, UserException;
	...
	SSerializerPluginConfiguration getSerializerByContentType(
			@WebParam(name = "contentType", partName = "getSerializerByContentType.contentType") String contentType)
			throws ServerException, UserException;

	/**
	 * @param name
	 *            Name of the QueryEngine
	 * @return QueryEngine
	 * @throws ServerException
	 *             , UserException
	 */
	...
	SQueryEnginePluginConfiguration getQueryEngineByName(
			@WebParam(name = "name", partName = "getQueryEngineByName.name") String name)
			throws ServerException, UserException;

	/**
	 * @param oid
	 *            ObjectID of the QueryEngine
	 * @return QueryEngine
	 * @throws ServerException
	 *             , UserException
	 */
	...
	SQueryEnginePluginConfiguration getQueryEngineById(
			@WebParam(name = "oid", partName = "getQueryEngineById.oid") Long oid)
			throws ServerException, UserException;
	...
	SDeserializerPluginConfiguration getSuggestedDeserializerForExtension(
			...
			@WebParam(name = "poid", partName = "getSuggestedDeserializerForExtension.poid") Long poid)
			throws ServerException, UserException;
	...
	void addExtendedDataToRevision(
			...
			@WebParam(name = "extendedData", partName = "addExtendedDataToRevision.extendedData") SExtendedData extendedData)
			throws ServerException, UserException;

	/**
	 * @param oid
	 *            ObjectID of the ExtendedData
	 * @return ExtendedData
	 * @throws ServerException
	 *             , UserException
	 */
	...
	SExtendedData getExtendedData(
			@WebParam(name = "oid", partName = "getExtendedData.oid") Long oid)
			throws ServerException, UserException;
	...
	SProjectSmall getProjectSmallByPoid(
			@WebParam(name = "poid", partName = "getProjectSmallByPoid.poid") Long poid)
			throws ServerException, UserException;
	...
	/**
	 * Undelete a previously deleted Project, Projets can be deleted with the
	 * deleteProject method
	 * 
	 * @param poid
	 *            The ObjectID of the Project to undelete
	...
	/**
	 * Branch a given Revision as a new Revision on a new Project, branching is
	 * always synchronous
	 * 
	 * @param roid
	 *            ObjectID of the Revision to branch
	 * @param projectName
	 *            Name of the to be created Project
	 * @param comment
	 *            A comment describing the new Revision
	 * @return The result of this branch, you can use getCheckinState with this
	 *         ID
	 * @throws ServerException
	 *             , UserException
	 */
	...
	Long branchToNewProject(
			...
			@WebParam(name = "sync", partName = "branchToNewProject.sync") Boolean sync)
			throws ServerException, UserException;

	/**
	 * Branch a given Revision as a new Revision in the given Project, branching
	 * is always synchronous
	 * 
	 * @param roid
	 *            ObjectID of the Revision to branch
	 * @param destPoid
	 *            ObjectID of the Project to which a branch should be made
	 * @param comment
	 *            A comment describing the new Revision
	 * @return The result of this branch, you can use getCheckinState with this
	 *         ID
	...
	/**
	 * @param name
	 * @return A list of projects with the given name (can be multiple because
	 *         subprojects are also returned)
	 * @throws ServerException
	 *             , UserException
	 */
	@WebMethod(action = "getProjectsByName")
	List<SProject> getProjectsByName(
			@WebParam(name = "name", partName = "getProjectsByName.name") String name)
			throws ServerException, UserException;
	...
	/**
	 * @param oid
	 *            ObjectID of the ExtendedDataSchema
	...
	SExtendedDataSchema getExtendedDataSchemaById(
			@WebParam(name = "oid", partName = "getExtendedDataSchemaById.oid") Long oid)
			throws ServerException, UserException;
	...
	List<SRevision> getAllRevisionsOfProject(
			@WebParam(name = "poid", partName = "getAllRevisionsOfProject.poid") Long poid)
			throws ServerException, UserException;
	...
	List<SProject> getAllProjects(
			...
			@WebParam(name = "onlyActive", partName = "getAllProjects.onlyActive") Boolean onlyActive)
			throws ServerException, UserException;
	 ...
	 * 
	 * @param projectName
	 *            Name of the project, must be a unique name within all
	 *            root-projects
	...
	SProject addProject(
			...
			@WebParam(name = "schema", partName = "addProject.schema") String schema)
			throws ServerException, UserException;
	
	/**@WebParam(name = "projectName", partName = "addProject.projectName") String projectName,
			@WebParam(name = "schema", partName = "addProject.schema") String schema,
			@WebParam(name = "description", partName = "addProject.description") String description)
			throws ServerException, UserException;**/

	/**
	 * @add by ch Interface add for test
	 * @param test
	 *            content
	 * @return string
	 * @throws ServerException
	 *             , UserException
	 */
	@WebMethod(action = "testFunction")
	String testFunction(
			@WebParam(name = "projectName", partName = "testFunction.projectName") String projectName,
			@WebParam(name = "schema", partName = "testFunction.schema") String schema)
			throws ServerException, UserException;
	 ...
	 * 
	 * @param projectName
	 *            Name of the project, must be a unique name within the parent
	 *            project
	 * @param parentPoid
	 *            The ObjectID of the parent project
	 * @return The created Project
	 * @throws ServerException
	 *             , UserException
	 */
	...
	SProject addProjectAsSubProject(
			...
			@WebParam(name = "schema", partName = "addProjectAsSubProject.schema") String schema)
			throws ServerException, UserException;

	/**
	 * Delete a Project, Projects can be undeleted with the undeleteProject
	 * method
	 * 
	 * @param poid
	 *            ObjectID of the Project to delete
	...
	SExtendedDataSchema getExtendedDataSchemaByNamespace(
			@WebParam(name = "namespace", partName = "getExtendedDataSchemaByNamespace.namespace") String namespace)
			throws UserException, ServerException;

	@WebMethod(action = "getAllProjectsSmall")
	List<SProjectSmall> getAllProjectsSmall() throws ServerException,
			UserException;
