...
public class JsonHandler {
	...
	private void processSingleRequest(JsonObject request, String jsonToken, HttpServletRequest httpRequest, JsonWriter writer) throws Exception {
		...
		String methodName = request.get("method").getAsString();//这里才刚获取到接口名和方法名
		...
		SMethod method = sService.getSMethod(methodName);//获取方法
		if (method == null) {
		...
		if (request.has("parameters")) {
			...
			for (int i = 0; i < method.getParameters().size(); i++) {
				SParameter parameter = method.getParameter(i);//到这里了
		...
		PublicInterface service = getServiceInterface(httpRequest, bimServer, sService.getInterfaceClass(), methodName, jsonToken);//@ch get InterfaceImpl
		Thread.currentThread().setName(interfaceName + "." + methodName);
		Object result = method.invoke(sService.getInterfaceClass(), service, parameters);//@ch 在这里调用接口 parm分别是接口名，服务，参数
		...
		if (writer != null) {
			if (result == null) {
				...
				writer.beginObject();
				writer.name("result");
				converter.toJson(result, writer);//writer为result写入的内容
				writer.endObject();
			}
		}
		long e = System.nanoTime();
		LOGGER.debug(interfaceName + "." + methodName + " " + ((e - s) / 1000000) + "ms");//time
	}
