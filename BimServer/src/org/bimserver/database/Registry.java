...
public class Registry {
	...
	// BinUtils.byteArrayToInt(bytes);将字节数组转化成Int，也可以是String
	public int readInt(String key, DatabaseSession databaseSession, int defaultValue) throws BimserverLockConflictException, BimserverDatabaseException {
		byte[] bytes = keyValueStore.get(REGISTRY_TABLE, key.getBytes(Charsets.UTF_8), databaseSession);//读取INT-Registry表的INT-Registry值
		if (bytes == null) {
			return defaultValue;
		} else {
			return BinUtils.byteArrayToInt(bytes);
