...
//÷¥––action
	...
	public String execute() throws UserException, BimserverLockConflictException, BimserverDatabaseException, ServerException {
		...
		if (user != null) {
			...
			//@ch
			//get password from database and check it by psw hash from web
			if (new Authenticator().validate(password, user.getPasswordHash(), user.getPasswordSalt())) {//—È÷§
		...
		//throw new UserException("µ«¬º ß∞‹£¨’ÀªßªÚ√‹¬Î¥ÌŒÛ");
	}
}