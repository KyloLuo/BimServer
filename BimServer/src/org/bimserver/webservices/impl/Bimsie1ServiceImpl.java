...
//			org.bimserver.plugins.serializers.Serializer serializer = getBimServer().getEmfSerializerFactory().get(serializerOid).createSerializer(new org.bimserver.plugins.serializers.PluginConfiguration());
	...
	@Override
	public List<SRevision> getAllRevisionsOfProject(Long poid) throws ServerException, UserException {//@ch 获得项目所有revisions
		requireRealUserAuthentication();//@ch 验证用户权限以及服务器状态
		DatabaseSession session = getBimServer().getDatabase().createSession();
		try {
	...
	public SProject addProject(String projectName, String schema) throws ServerException, UserException {
		...
		try {
			...
			//test 
			SProject project = getBimServer().getSConverter().convertToSObject(session.executeAndCommitAction(action));
			//project.testString ="456";
			return project;
			//原始数据
			//return getBimServer().getSConverter().convertToSObject(session.executeAndCommitAction(action));
		} catch (Exception e) {
	...
	@Override
	public String testFunction(String projectName, String schema) throws ServerException, UserException {
		requireRealUserAuthentication();
		DatabaseSession session = getBimServer().getDatabase().createSession();
		return "I am coming back";
	}
