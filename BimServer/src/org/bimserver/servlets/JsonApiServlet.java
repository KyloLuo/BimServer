...
public class JsonApiServlet extends SubServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			JsonElement parse = parser.parse(jsonReader);//@ch parser Ω‚ Õ json project
			if (parse instanceof JsonObject) {  //@ch if this is a json object, then handler it 
				JsonObject jsonRequest = (JsonObject) parse;
				response.setHeader("Content-Type", "application/json");
				getBimServer().getJsonHandler().execute(jsonRequest, request, response.getWriter());//@ch excute
			} else {
